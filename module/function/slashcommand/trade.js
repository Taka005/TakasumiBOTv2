...
module.exports = async(interaction)=>{
  ...
  if(interaction.commandName === "trade"){
    ...
    if(interaction.options.getSubcommand() === "buy"){
      ...
      await db(`UPDATE count SET buy = buy + 1, stock = stock + ${Math.floor(count*0.03)} WHERE id = ${process.env.ID}`);
      ...
      await db(`UPDATE count SET sell = sell + 1, stock = stock - ${Math.floor(count*0.03)} WHERE id = ${process.env.ID}`);
      await db(`UPDATE money SET stock = ${data.stock - count} WHERE id = ${interaction.user.id}`);
