module.exports = async(interaction)=>{
  ...
  if(interaction.commandName === "status"){
    ...
    const guild = await fetchGuildCount(interaction.client) - (await db(`SELECT guild FROM log WHERE DATE(time) = "${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()}" ORDER BY time ASC LIMIT 1;`))[0].guild;
    const user = await fetchUserCount(interaction.client) - (await db(`SELECT user FROM log WHERE DATE(time) = "${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()}" ORDER BY time ASC LIMIT 1;`))[0].user;
