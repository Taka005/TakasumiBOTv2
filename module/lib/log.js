...
module.exports = {
  ...
  "reset":()=>{
    if(fs.existsSync("./tmp/app.log")){
      fs.copyFileSync("./tmp/app.log",`./tmp/app-${new Date().toLocaleString("sv-SE")}.log`);
    }

    if(fs.existsSync("./tmp/db.log")){
      fs.copyFileSync("./tmp/db.log",`./tmp/db-${new Date().toLocaleString("sv-SE")}.log`);
    }
