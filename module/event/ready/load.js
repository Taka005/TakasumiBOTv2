module.exports = async(client)=>{
  ...
  cron.schedule("0 * * * *",async()=>{
    ...
    await db("DELETE FROM history WHERE time < DATE_SUB(NOW(),INTERVAL 1 DAY);");
  ...
  cron.schedule("*/15 * * * *",async()=>{
    ...
    price -= Math.round((Math.random()*150 + 1)*correct*Math.abs(correct)+(Math.random()*25 + 1)*Math.sin(Math.random()*2*Math.PI));
